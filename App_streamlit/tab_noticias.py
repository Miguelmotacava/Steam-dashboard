...
def render_noticias(df_super):
    ...
    with col_n2: filtro_tiempo = st.radio("⏱️ Rango temporal:", ["Mes", "Todo"], index=1)
    with col_n3: tipo_noticia = st.radio("🛠️ Tipo:", ["Todo", "Parches", "Anuncios"])

    appid_elegido = df_super[df_super['nombre'] == juego_elegido]['appid'].iloc[0]
    df_news = load_news_data(appid_elegido)
    
    if not df_news.empty:
        if filtro_tiempo == "Mes": df_news = df_news[df_news['fecha_dt'] >= (pd.Timestamp.now() - pd.Timedelta(days=30))]
        if tipo_noticia == "Parches": df_news = df_news[df_news['feed_type'] == 1]
        elif tipo_noticia == "Anuncios": df_news = df_news[df_news['feed_type'] == 0]
        ...
        if not df_news.empty:
            ...
            with col_m1:
                conteo_cats = df_news['feedlabel'].fillna('Otros').value_counts().sort_values(ascending=True)
                fig_m1, ax_m1 = plt.subplots(figsize=(3, 2))
                fig_m1.patch.set_alpha(0.0) 
                ax_m1.patch.set_alpha(0.0)  
                ax_m1.barh(conteo_cats.index.astype(str), conteo_cats.values, color=RED_BASE)
            ...
            with col_m2:
                conteo_temp = df_news.groupby(df_news['fecha_dt'].dt.date).size().sort_index()
                ...
                ax_m2.plot(range(len(conteo_temp)), conteo_temp.values, color=RED_BASE, marker='o', markersize=4)
                ax_m2.set_xticks(range(len(conteo_temp)))
                ax_m2.set_xticklabels(x_labels, rotation=30, ha='right')
                ...
                ax_m2.tick_params(colors='gray', labelsize=8)
